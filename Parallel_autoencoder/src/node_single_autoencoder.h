...
namespace parallel_autoencoder{
	...
	public:
		...
		node_single_autoencoder(const my_vector<int>& _layers_size, std::default_random_engine& _generator,
			...
			: node_autoencoder(_layers_size, _generator, 0, 0, 0, _oslog, 0)
			{
			...
			for(uint layer_number = trained_rbms; layer_number != number_of_rbm_to_learn; layer_number++)
			{
				...
				auto& visible_biases =  layer_biases[index_reverse_layer];
			    {
					...
					for(uint epoch = 0; epoch != rbm_n_training_epocs; epoch++){
						...
						float learning_rate = GetRBMLearningRate(epoch, layer_number);
				...
				my_vector<float>& hidden_biases, matrix<float>& weights)
		{
			...
			while(smp_manager.get_next_sample(input_samples, sample_extension, &sample_filename)){
				...
				smp_manager.save_sample(output_samples, true, new_image_path_folder, sample_filename + ".jpg" ); //dati in formato immagine
			}
		...
		void fine_tuning()
		{
			...
			for(uint epoch = 0; epoch != fine_tuning_n_training_epocs; epoch++)
			{
				...
				//per ciascun esempio...
				while(smp_manager.get_next_sample(activation_layers[0], ".jpg")){
		...
		my_vector<float> reconstruct()
		{
			...
			string file_name = "";
			smp_manager.get_next_sample(activation_layers[0], ".jpg", &file_name);
