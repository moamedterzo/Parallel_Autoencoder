...
namespace parallel_autoencoder
{
	...
	void node_autoencoder::calc_comm_sizes(const GridOrientation current_or,
			my_vector<MPI_Comm_MasterSlave>& acc_gridcolrow_comm, const bool calc_for_acc,
			const uint my_k_col_row_number,	const int n_tot_vishid_units)
	{
	...
	float node_autoencoder::get_learning_rate_rbm(const uint epoch_number, const uint layer_number)
	{
	...
	void node_autoencoder::execute_command(CommandType command)
	{
		...
		switch(command){
			...
			case CommandType::train:
				...
				oslog << "Learning RBMs ...\n";
				oslog << "RBMs to train: " <<  number_of_rbm_to_learn <<"\n";
				oslog << "RBMs already trained: " << trained_rbms << "\n";
				oslog << "Number of final layers: " <<  number_of_final_layers <<"\n";
				oslog.flush();
	...
	void node_autoencoder::set_size_for_layers(const my_vector<int>& _layers_size_source)
	{
		...
		for(uint i = 0; i !=_layers_size_source.size(); i++)
		{
		   ...
		   //si determina la grandezza del layer da ricostruire
	...
	void node_autoencoder::set_orientation_for_layers()
	{
		...
		for(uint i = 0; i != number_of_rbm_to_learn; i++)
		{
			...
			//numero di nodi di input e di output è invertito tra il layer di encoding e quello di decoding,
